...
function main(arr, index = 0, acc = 0) {
	if(arr[index] === arr[arr.length - 1]) return acc + (arr[index].length * 2);
	else return (arr[index].length * 2) + main(arr, index + 1, acc);
}
...
function main2(arr) {
	index += 1;
	if(arr[index] === arr[arr.length - 1]) return acc + (arr[index].length * 2);
	else return (arr[index].length * 2) + main2(arr);
}

console.log('Result5:', main2(splited));

const end = new Date().getMilliseconds();

console.log('Time5:', end - start);
