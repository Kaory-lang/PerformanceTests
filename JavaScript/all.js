...
let result = text.split(' ').map((curr) => curr.length).map((curr) => curr * 2).reduce((acc, curr) => acc + curr);
...
// Declarative or Functional (RECURSION)
// Note:
// Not Eficient because create a lot of copys and edit to much arrays... Really slow.
// Better recursion in recursion2.js.
const start3 = new Date().getMilliseconds();

let splited2 = text.split(' ');

function main(array) {
	if(array.length === 1) return array[0].length * 2;
	else return (array[0].length * 2) + main([...array].slice(1));
}

console.log('Result3:', main(splited2));

const end3 = new Date().getMilliseconds();

console.log('Time3:', end3 - start3);

// Declarative or Functional (NO RECURSION, FULL FUNCTIONAL)
const start4 = new Date().getMilliseconds();
...
function splitStr(str){
	...
	for(let letter in str){
		if(str[letter] === ' ' || Number(letter) === str.length - 1) {
			result = (Number(letter) === str.length - 1) ? [...result, cache + str[letter]] : [...result, cache];
			cache = '';
		}else cache += str[letter];
	};
...
function mapLength(arr) {
	let result = [];
	for(let word of arr) result = [...result, word.length];
	return result;
}

function quadraticRoot(arr) {
	let result = [];
	for(let number of arr) result = [...result, number * 2]; 
	return result;
}
...
console.log('Result4:', reduceArr(quadraticRoot(mapLength(splitStr(text)))));

const end4 = new Date().getMilliseconds();

console.log('Time4:', end4 - start4);
